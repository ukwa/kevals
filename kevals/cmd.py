...
def main():
    # Set up a parser:
    parser = argparse.ArgumentParser(prog='kevals')

    # Common arguments:
    common_parser = argparse.ArgumentParser(add_help=False)
    common_parser.add_argument('-v', '--verbose',  action='count', default=0, help='Logging level; add more -v for more logging.')

    common_parser.add_argument('--dry-run', action='store_true', help='Do not modify the database.')
    common_parser.add_argument('-i', '--indent', type=int, help='Number of spaces to indent when emitting JSON.')
    common_parser.add_argument('-k', '--kevals-url', type=str, help='The Kevals service URL to talk to.', required=True)
    ...
    parser_get = subparsers.add_parser(
        'get', 
        help='Get a single record from the KevalsDB.',
    ...
    parser_import = subparsers.add_parser(
        'import', 
        help='Import JSONL documents into a KevalsDB.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=[common_parser])
    ...
    parser_list = subparsers.add_parser(
        'list', 
        help='Get a list of records from the KevalsDB, output as JSONL by default.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    ...
    parser_up = subparsers.add_parser(
        'update', 
        help='Create or update on a record in the KevalsDB.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=[common_parser])
    ...
    # Set up Solr client:
    tdb = SolrKevalsDB(args.kevals_url)

    # Ops:
    logger.debug("Got args: %s" % args)
    if args.op == 'list':
        for doc in tdb.list(args.field, limit=args.limit):
            if args.ids_only:
                print(doc['id'])
            else:
